...
<path d="M16.29 6.29 10.59 12l5.7 5.71 1.42-1.42-4.3-4.29 4.3-4.29z"/>
    ...
    <div class="cards-grid">
        ...
        if ($result->num_rows === 0) {
            echo "<p style='color: #f8fafc; text-align: center; grid-column: 1 / -1;'>Aucun livre enregistré pour le moment.</p>";
        } else {
            while ($livre = $result->fetch_assoc()) {
                ...
                echo '<div class="inner-border-overlay" data-tilt-transform-element></div>';
                
                echo '<div class="content-area p-4 sm:p-5 lg:p-7" data-tilt-transform-element>';
                echo '<div class="gradient-overlay"></div>';
                
                echo '<div class="elevation-badge" data-tilt-transform-element>';
                echo '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="w-4 h-4">';
                echo '<path d="M12 2L6.5 12h11L12 2zm5.5 11l-5.5 9-5.5-9h11z"/>';
                echo '</svg>';
                echo htmlspecialchars($livre['Annee']);
                echo '</div>';
                
                echo '<div class="text-block" data-tilt-transform-element>';
                echo '<h1 class="font-serif-display text-3xl sm:text-4xl lg:text-5xl font-bold mb-2">';
                echo '<svg class="mountain-icon" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor">';
                echo '<path d="M14.344 6.219l-2.344-3.219-2.344 3.219-4.656 7.781h14l-4.656-7.781zm-7.344 9.781l-1 2h10l1.625-2.708-1.501-.292-1.124 1h-4l-1.125-1-1.875.001zm11.69-1l-.69.999 1.311 1.311 1.689.69-1.311-1.311-.689-.69z"/>';
                echo '</svg>';
                echo htmlspecialchars($livre['Titre']);
                echo '</h1>';
                echo '<p class="text-sm sm:text-base lg:text-lg font-light">';
                echo htmlspecialchars($livre['Auteur']) . ' • ' . (isset($livre['Genre']) ? htmlspecialchars($livre['Genre']) : 'Genre non spécifié');
                echo '</p>';
                echo '</div>';
                
                echo '<button class="tour-button" data-tilt-transform-element onclick="showInfos(this)">';
                echo 'Voir détails';
            ...
            const details = `
                <div class="text-block" data-tilt-transform-element>
                    <h1 class="font-serif-display text-3xl sm:text-4xl lg:text-5xl font-bold mb-2">
                        ${textBlock.querySelector('h1').innerHTML}
                    </h1>
                    <p class="text-sm sm:text-base lg:text-lg font-light mb-4">
                        ${textBlock.querySelector('p').innerHTML}
                    </p>
                    <div style="background: rgba(255,255,255,0.1); padding: 1rem; border-radius: 0.5rem; margin: 1rem 0;">
                        <p style="margin: 0.5rem 0;"><strong>Année de publication:</strong> ${card.querySelector('.elevation-badge').textContent.trim()}</p>
                        <p style="margin: 0.5rem 0;"><strong>Genre:</strong> ${textBlock.querySelector('p').textContent.split('•')[1] || 'Non spécifié'}</p>
                    </div>
                </div>
            `;
            
            textBlock.innerHTML = details;
            tourButton.textContent = 'Fermer';
